...
def main():
    ...
    ids_with_parent = collect_name_isos(candidates, candidate_io_subs, progress=args.progress)
    # for i, specs in sub_specializations.items():
    #     print(f"{i}\t: {len(specs)}")

    if DROP:
        candidates = [x for i, x in enumerate(candidates) if i not in ids_with_parent]
        UPDATE_IDS = True
        if UPDATE_IDS:
